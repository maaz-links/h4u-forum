...
<div class="container my-5 card px-4 py-4">
    ...
    <form action="{{ route('shop.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            <label>Title</label>
            <input type="text" name="title" value="{{ $shop->title }}" placeholder="Enter title here"
        ...
        <div class="mb-3">
            <label>Price</label>
            <input type="number" min='0' step="0.01" name="price" value="{{ $shop->price }}" placeholder="Enter price here eg:10"
                class="form-control" required>
            @error('price')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="mb-3">
            <label>Credits</label>
            <input type="number" name="credits" value="{{ $shop->credits }}" placeholder="Enter credits eg:50"
                class="form-control" required>
            @error('credits')
            <span class="text-danger">{{$message}}</span>
            @enderror
